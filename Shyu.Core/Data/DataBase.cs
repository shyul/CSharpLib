...
namespace Shyu
{
    public partial class DBUtil
    {
        ...
        public static void InitDataBaseFile(FileInfo DataFile)
        {
            ...
            {
                string TemplatePath = AppDomain.CurrentDomain.BaseDirectory + @"\Data\Empty.mdf";
                File.Copy(TemplatePath, DataFile.FullName, true);
            }
            if (File.Exists(FilePath + @"\" + FileName + "_log.ldf"))
                File.Delete(FilePath + @"\" + FileName + "_log.ldf");
        }
        public static long LastEID(FileInfo DataFile, string TableName)
        {
            if (DBUtil.CheckExistTable(DataFile, TableName))
            {
                StringBuilder s = new StringBuilder();
                s.AppendFormat("select TOP 1 [EID] from [{0}] ORDER BY [EID] DESC;", TableName);
                string ResultStr = string.Empty;
                using (SqlConnection conn = new SqlConnection())
                {
