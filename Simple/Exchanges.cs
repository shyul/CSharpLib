...
namespace Shyu
{
    ...
    public partial class TechDataBase
    {
        ...
        private void InitTables(string SymbolName)
        {
            ...
            Table_Ratios.Columns.Add(new DataColumn("NAME", typeof(string))); // "NTEXT"
            Table_Ratios.Columns.Add(new DataColumn("VALUE", typeof(double))); // "NTEXT"
            Table_Ratios.Columns.Add(new DataColumn("SOURCE", typeof(char))); // "CHAR"
        ...
        private void WriteDataBase()
        {
            if (!Table_EOD.TableName.Contains('_'))
            {
                ...
                {
                    SqlCmd = "CREATE TABLE [dbo].[" + Table_EOD.TableName + "] ([EID] BIGINT NOT NULL, [NAME] NTEXT NOT NULL, [VALUE] FLOAT NULL, [SOURCE] CHAR(1) NOT NULL)";
            ...
            //while (p < lineCount)
            {
                ...
                if (line.Length > 9)
                {
                    ...
                    if (val.Length == 14)
                    {
                        ...
                        try { Row["VOLUME"] = Convert.ToInt64(val[6]); } catch { Row["VOLUME"] = 0; }
